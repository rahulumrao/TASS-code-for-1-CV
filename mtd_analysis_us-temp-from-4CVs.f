...
!Make CV file containing CV1(t), CV2(t) from CONSTRAINT and cvmdck_mtd
     ...
     &         width(mtd_steps))
      ...
      DO i_md=1,md_steps
!read 4 CVs
        READ(14,*)dummy1,dummy1,dummy11,dummy11,dummy11,dummy11,
     &            cv2(i_md),cv1(i_md) !,cv3(i_md)
        WRITE(16,'(I10,2F16.6)')i_md,cv1(i_md),cv2(i_md) !,cv3(i_md)
      END DO
...
!     den=den+1.D0
      ...
      den=1.D0/dum
      OPEN(2,FILE='Pu_temp.dat',STATUS='unknown')
      DO i_s1=1,nbin1
