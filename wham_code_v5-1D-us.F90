...
if (parent) then
   ...
   do i_umbr=1, umbr_n 
      ...
      do i_s1=1,nbin1 !US
!         do i_s2=1,nbin2 !MTD
         read(3,*)dummy,biased_prob(i_s1,i_umbr)
!         end do
      end do
   enddo
end if
...
call RBcast(biased_prob,nbin1*1*umbr_n)
...
!PERFORMS WHAM.
 ...
 write(*,*)'new_grid', gleng1_min, gleng1_max,gleng2, rank, grid(1,1)
  ...
  scf_loop : do
       iter=iter+1
       call wham_scf(a,umbr_k,umbr_mean,nmax,grid0,biased_prob,umbr_n,nbin1,1,ncv,kt,prob,cnvg,gleng1_min,gleng1_max,1)
       if (parent) write(*,*) 'iteration #', iter, 'convergence =',cnvg
       if (mod(iter,100) .eq. 0 ) then
       call print_pmf(ncv,nbin1,1,grid0,kt,prob,parent)
       endif
